...
$token = $_GET['token'] ?? '';
$consulta = "SELECT * FROM usuario WHERE token = '$token'";
...
if ($result->num_rows > 0) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $actualizar = "UPDATE usuario SET Contraseña = '$nueva_contraseña_encriptada', token = NULL WHERE token = '$token'";
        $conn->query($actualizar);

        echo " <script> alert('Contraseña restablecida correctamente.')
        window.location.href='/index.php';</script>";
    }
} else {
  echo " <script> alert('Enlace no valido.') </script>";
}
?>
  ...
  <main>
    ...
    <div class="formulario">
      <form class="form" name="contact-form" action="" method="POST">
        ...
        <div class="form-body">
          <div>
            <label class="label" for="nueva_contraseña">Ingresa tu nueva contraseña:</label>
            <input class="input-text" type="password" name="nueva_contraseña" id="nueva_contraseña" required>
          </div>

</div>
<input class="button" type="submit" value="Restablecer" name="Restablecer" id="Restablecer">
      </form>
    </div>
  </main>
<script type="text/javascript"  src="/Scripts/recuperar.js"></script>
<footer>
    <ul class="footer__ul">
